...
\begin{itemize}
    \item \textbf{16 corners} (4-color pieces)
    \begin{itemize}
        \footnotesize
        \item 12 orientations each
    \end{itemize}
    \item \textbf{32 faces} (3-color pieces)
    \begin{itemize}
        \footnotesize
        \item 6 orientations each
    \end{itemize}
    \item \textbf{24 edges} (2-color pieces)
    \begin{itemize}
        \footnotesize
        \item 2 orientations each
    \end{itemize}
\end{itemize}

\vspace{0.5em}

\textbf{Each quarter turn affects:}
\begin{itemize}
    \item 8 corners, 12 faces, 4 edges
\end{itemize}
...
This rich group structure enables complex dynamics!
\end{columns}
\end{frame}
...
\begin{block}{Group-Theoretic View}
Let $G$ be the puzzle group. Each move $m_i \in G$ is a bijection with $m_i^{-1} \circ m_i = \text{id}$.

The sequence generates a cyclic subgroup:
$$\langle T_M \rangle = \{T_M^0, T_M^1, \ldots, T_M^{p-1}\} \subseteq G$$
...
\textbf{Monoflip Invariant (unique to 4D):} A single 4c piece can be "flipped" with all others solved, related to $A_4$ orientation group.
\end{frame}
...
\footnotesize
Continuous systems: $\lambda = \lim_{t \to \infty} \frac{1}{t} \ln \frac{||\delta(t)||}{||\delta(0)||}$
...
\For{$n = 1, 2, \ldots, N_{\max}$}
    \State $s \gets T_M(s)$ \Comment{Apply sequence}
    \State $h \gets \text{SHA256}(s)$ \Comment{Hash state}
    \If{$h \in \text{visited}$}
        \State \Return $n - \text{visited}[h]$ \Comment{Period}
    \EndIf
    \State $\text{visited}[h] \gets n$
\EndFor
\end{algorithmic}
\textbf{Complexity:} $O(p \cdot (T_M + H))$ where $p$ = period
\end{block}
\end{frame}
